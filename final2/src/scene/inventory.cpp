...
Inventory::Inventory(GLWidget277 *context):Drawable(context),frameLW(glm::ivec2(1)),blockList(),selected_BlockType(0)
{
    ...
    blockNum[IronOre]=0;
    blockNum[Flower]=0;
    blockNum[GrassLand]=0;
    blockNum[Snow]=0;
    blockNum[SnowFlake];
    blockNum[Cactus]=0;
    blockNum[Fire]=0;
    blockNum[WoodOnFire]=0;
    blockNum[SnowSpike]=0;
...
void Inventory::create()
{
    ...
    //Front Face
        ...
        if(i==selected_BlockType)
            itemframeUV=glm::vec4(15,11,0.2,4);
        else
            itemframeUV=glm::vec4(14,11,0.2,4);
    ...
    for(unsigned int i=0;i<blockList.size();i++)
    {
        if(blockList[i]==Water)
            blockUV=glm::vec4(14,12,0.2,4);
        else if(blockList[i]==GrassLand)
            blockUV=glm::vec4(3,0,2 ,4);
        else if(blockList[i]==Stone)
            blockUV=glm::vec4(1,0,0.4,4);
        else if(blockList[i]==Lava)
            blockUV=glm::vec4(14,14,0.2,4);
        else if(blockList[i]==Dirt)
            blockUV=glm::vec4(2,0,1,4);
        else if(blockList[i]==Wood)
            blockUV=glm::vec4(4,1,2,4);
        else if(blockList[i]==Coal)
            blockUV=glm::vec4(2,2,3,4);
        else if(blockList[i]==IronOre)
            blockUV=glm::vec4(2,1,1,4);
        else if(blockList[i]==Leaf)
            blockUV=glm::vec4(5,3,1,4);
        else if(blockList[i]==Flower)
            blockUV=glm::vec4(12,0,1,4);
        else if(blockList[i]==Default)
            blockUV=glm::vec4(2,0,2,4);
        else if(blockList[i]==Grass)
            blockUV=glm::vec4(7,2,1,4);
        else if(blockList[i]==Snow)
            blockUV=glm::vec4(4,4,0.3,4);
        else if(blockList[i]==SnowFlake)
            blockUV=glm::vec4(11,0,1,4);
        else if(blockList[i]==Cactus)
            blockUV=glm::vec4(6,4,1,4);
        else if(blockList[i]==Fire)
            blockUV=glm::vec4(8,12,0.02,4);
        else if(blockList[i]==WoodOnFire)
            blockUV=glm::vec4(4,1,1,4);
        else if(blockList[i]==SnowSpike)
            blockUV=glm::vec4(2,11,0.1,4);
        //Front Face
            glm::vec3 frontN(0,0,1);
            glm::vec3 frontT=glm::cross(glm::vec3(0,1,0),frontN);
            float px=startx+lengthX*2*i;
